...
void libfunc_totalarguments(void){
    ...
    //painrw tin prohgoumeni timi tou topsp
    unsigned p_topsp=avm_get_envvalue(topsp+AVM_SAVEDTOPSP_OFFSET);
	avm_memcellclear(&retval);
	
	if(!p_topsp){//kai an einai miden simainei oti den exw klithei mesa se synartisi ara error
		avm_error("'totalarguments' called outside a function!");
		retval.type=nil_m;
	}
	else{
		retval.type=number_m;
        //painrei apo to environment to totalactuals
        //kai to espitrefei
...
void libfunc_argument(void){
    //painrei tin prohgoumeni timi tou topsp
    unsigned p_topsp=avm_get_envvalue(topsp+AVM_SAVEDTOPSP_OFFSET);
	avm_memcellclear(&retval);
    //elegxos 1
	//an einai miden
	if(!p_topsp){//akrivws idio me prin kanoume elegxo oti i totalarguments aki i argument exei klithei mesa se synartisi
		avm_error("'argument' called outside a function!");
		retval.type=nil_m;
	}
	else{
        //pairnw me to posa orismata dothikan stin argument
        unsigned n=avm_totalactuals();
        //elegxos2
        if(n!=1)//kai an den einai 1 orisma petaw lathos
            avm_error("one argument (not %d) expected in 'argument'!",n);
        else{
            avm_memcellclear(&retval);

            //pairnw ton arithmo me ton opoio kalese o xristis tin argument
            avm_memcell *argument_parameter = avm_getactual(0);
            
            //apothikevw ayti ti timi se mia metavliti pou onomazw index
            int index = argument_parameter->data.numVal;//poio parameter theloume
            //lecture 12 stack[topsp + Κ + sym->offset + 1]
            int K = AVM_STACKENV_SIZE;//exei timi 4
            //paw stin stoiva kai pairnw to i-osto argument
            avm_memcell *function_parameter = &stack[p_topsp + K + index + 1];


            //mesw tou retval ana logws ti typo eisai epistrefeis ayto to argument
            if(function_parameter->type==number_m){//px an itan arithmos ayto to i-osto parameter
                retval.type=number_m;//theteis edw typo tou retval arithmos
		        retval.data.numVal=function_parameter->data.numVal;//kai vazeis kai tin timi
                return;//telos
            }
